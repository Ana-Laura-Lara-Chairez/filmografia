          ...
          <div className="flex sm: flex-col lg:flex-row">
            ...
            <div className="w-2/2 p-5">
              <header className="w-full">
                <p> <span className="text-md font-bold mb-3 mt-10">Pais de origen: </span>{`${filmografia?.pais}`}  </p>
                <h1 className="text-4xl font-semibold mb-3 mt-5 text-blue-80">{titulo}</h1>
                <p> <span className="text-md font-bold mb-3 mt-10">Nombre original: </span>{title}</p>
                <button className="text-white bg-orange-80 px-2 py-1 mt-2 rounded-lg mb-5">{filmografia?.year}</button>

                <p> <span className="text-md font-bold">Protagonistas: </span>{filmografia?.protagonistas}</p>

                <div className="grid sm: grid-cols-1 md:grid-cols-2 gap-1 mt-5 mb-5">
                  <p> <span className="text-md font-bold">Dirección: </span>{filmografia?.direccion} </p>
                  <p> <span className="text-md font-bold">Duración: </span>{filmografia?.duracion} </p>
                  <p> <span className="text-md font-bold">Guion: </span>{filmografia?.guion} </p>
                  <p> <span className="text-md font-bold">Filmada en: </span>{filmografia?.filmada_en} </p>
                  <p> <span className="text-md font-bold">Fotografía: </span>{filmografia?.fotografia} </p>
                  <p> <span className="text-md font-bold">Música: </span>{filmografia?.musica} </p>
                  <p> <span className="text-md font-bold">Productora: </span>{filmografia?.productora} </p>
                </div>

                <p className="text-md font-bold">Sinópsis: </p>
                <p className='mt-3 mb-3'>{filmografia?.sipnopsis}.</p>

                <p className="text-md font-bold">Synopsis: </p>
